...
\cmdlink{intro:genclone:as.genclone}{as.genclone} & Converts genind objects to genclone objects \\
\cmdlink{function:genclone2genind}{genclone2genind} & Removes the \texttt{@mlg} slot from genclone objects \\
...
\texttt{strata} & \textit{(moved to adegenet)} Define a population hierarchy of a genclone object\\
