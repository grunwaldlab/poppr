...
Below is a table of functions found in *poppr*. These
functions are linked within the document. If a function name is blue,
simply click on it to go to its definition and description.
...
<span><span>**  – continued from previous page**</span></span>\
\
**Function** & **Description**\
\
...
<span> ``</span> & Read *GenAlEx* formatted csv files to a `genind`
object\
<span> ``</span> & Converts `genind` objects to *GenAlEx* formatted csv
files\
<span> ``</span> & Converts `genind` objects to `genclone` objects\
\
\
<span> ``</span> & Converts `genind` objects to `genclone` objects\
<span> ``</span> & Removes the `@mlg` slot from `genclone` objects\
`setPop` & *(moved to adegenet)* Set the population using defined
hierarchies\
`splitStrata` & *(moved to adegenet)* Split a concatenated hierarchy
imported as a population\
`strata` & *(moved to adegenet)* Define a population hierarchy of a
`genclone` object\
`strata` & *(moved to adegenet)* Extract the hierarchy data frame\
`addStrata` & *(moved to adegenet)* Add a vector or data frame to an
existing hierarchy\
`nameStrata` & *(moved to adegenet)* Rename a population hierarchy\
<span> ``</span> & Handles missing data\
<span> ``</span> & Clone censors at a specified population hierarchy\
<span> ``</span> & Detects and removes phylogenetically uninformative
loci\
<span> ``</span> & Subsets `genind` objects by population\
<span> ``</span> & Shuffles genotypes at each locus using four different
shuffling algorithms\
<span> ``</span> & recode polyploid data sets with missing alleles
imported as “0”\
`fix_replen` & Finds and fixes inconsistant repeat lengths for Bruvo’s
distance.\
\
\
`bruvo.dist` & Bruvo’s distance\
`diss.dist` & Absolute genetic distance (see `prevosti.dist`)\
...
`prevosti.dist` & Prevosti’s absolute genetic distance\
\
\
`aboot` & Creates a bootstrapped dendrogram for any distance measure\
`bruvo.boot` & Produces dendrograms with bootstrap support\
& based on Bruvo’s distance\
`diversity_boot` & Generates boostrap distributions of diversity\
& statistics for multilocus genotypes\
`diversity_ci` & Generates confidence intervals for\
...
`nmll` & Same as `mlg`.\
<span> ``</span> & Finds all multilocus genotypes that cross
populations\
<span> ``</span> & Returns a table of populations by multilocus
genotypes\
<span> ``</span> & Returns a vector of a numeric multilocus genotype
assignment for each individual\
`mlg.id` & Finds all individuals associated with a single multilocus
genotype\
`mlg.filter` & Collapses MLGs by genetic distance\
`filter_stats` & Calculate mlg.filter for all algorithms and plot\
`cutoff_predictor` & predict cutoff threshold from mlg.filter.\
...
`diversity_stats` & Creates a table of diversity indices for multilocus
genotypes.\
\
\
...
`samp.ia` & Index of association on random subsets of loci for genlight
objects.\
`poppr` & Returns a diversity table by population\
`poppr.all` & Returns a diversity table by population for all
compatible\
& files specified\
`private_alleles` & Tabulates the occurrences of alleles that only
occur\
& in one population.\
`locus_table` & Creates a table of summary statistics per locus.\
`rrmlg` & obtain round-robin estimates of multilocus genotypes.\
`rraf` & obtain round-robin estimates of allele frequencies.\
`pgen` & Genotype probability, $p_{gen}$.\
`psex` & Probability of observing a genotype more than once, $p_{sex}$.\
\
\
`imsn` & interactive construction and visualization of minimum spanning
networks\
`plot_poppr_msn` & Plots minimum spanning networks produced in poppr
with\
& scale bar and legend\
`greycurve` & Helper to determine the appropriate parameters for
adjusting the\
& grey level for msn functions\
`bruvo.msn` & Produces minimum spanning networks based off Bruvo’s
distance\
& colored by population\
`poppr.msn` & Produces a minimum spanning network for any pairwise
distance\
& matrix related to the data\
`info_table` & Creates a heatmap representing missing data or observed
ploidy\
`genotype_curve` & Creates a series of boxplots demonstrating how many
loci are\
& needed to represent the diversity of your data.\
\
\
`Aeut` & (AFLP) Oomycete root rot pathogen *Aphanomyces euteiches*\
& [@Grunwald:2006]\
`monpop` & (SSR) Peach brown rot pathogen *Monilinia fructicola*\
& [@everhart2014finescale]\
`partial_clone` & (SSR) partially-clonal data simulated via simuPOP\
& [@peng2008forward]\
`Pinf` & (SSR) Potato late blight pathogen *Phytophthora infestans*\
& [@goss2014irish]\
`Pram` & (SSR) Sudden Oak Death pathogen *Phytophthora ramorum*\
& [@kamvar2015spatial; @goss2009population]\
...
*Poppr* is an R package with convenient functions for
analysis of genetic data with mixed modes of reproduction including
sexual and clonal reproduction. While there are many R packages in CRAN
and other repositories with tools for population genetic analyses, few
are appropriate for populations with mixed modes of reproduction. There
are several stand-alone programs that can handle these types of data
sets, but they are often platform specific and often only accept
specific data types. Furthermore, a typical analysis often involves
switching between many programs, and converting data to each specific
format.

*Poppr* is designed to make analysis of populations with
mixed reproductive modes more streamlined and user friendly so that the
researcher using it can focus on data analysis and interpretation.
*Poppr* allows analysis of haploid and diploid
dominant/co-dominant marker data including microsatellites, Single
Nucleotide Polymorphisms (SNP), and Amplified Fragment Length
Polymorphisms (AFLP). To avoid creating yet another file format that is
specific to a program, *poppr* was created on the backbone
of the popular R package *adegenet* and can take all the
file formats that *adegenet* can take (Genpop, Genetix,
Fstat, and Structure) and newly introduces compatibility with GenAlEx
formatted files (exported to CSV). This means that anything you can
analyze in *adegenet* can be further analyzed with
*poppr*.

The real power of *poppr* is in the data manipulation and
analytic tools. *Poppr* has the ability to bootstrap,
clone-censor, and subset data sets. With *poppr* you can
also quickly calculate Bruvo’s distance, the index of association, and
easily determine which multilocus genotypes are shared across
populations.

In version 2.0, tools for genomic data were introduced
[@kamvar2015novel]. These tools allow researchers to define what it
means to be a clone lost in a sea of genomic data, generate bootstrapped
dendrograms with any genetic distance, and calculate minimum spanning
networks with reticulations to reveal the underlying population
structure of your clonal data.

A note about poppr version 2.0
------------------------------

In *poppr* version 1.1, the `genclone` object was
introduced. This was an extension of *adegenet*’s `genind`
object by adding two new slots:

1.  **mlg** numeric classification of multilocus genotypes in the data
    set

2.  **hierarchy** storage of multiple population factors.

In particular, the **hierarchy** slot played a large role in providing a
more tractable way of performing analyses such as clone-correction and
AMOVA. In March of 2015, The **hierarchy** slot was ported to the
*adegenet* packaged and the name was changed to the
**strata** slot. This means that code written before
*poppr* 2.0.0 might need some small changes. Please see the
<span style="font-variant:small-caps;">Migrating from poppr version
1</span> vignette for more details:
...
This vignette will cover all of the material you need to know to
efficiently analyze data in *poppr*. For information on
methods of analysis (eg. index of association, distance measures, AMOVA,
...), please read the manual pages provided for each function.

As *poppr* expanded from version 1.0, the vignette also
expanded to be 80+ pages. As a result, it became clear that over 22,000
was less of a manual and more of a novella with a terrible plot. To
remedy this, this vignette will focus only on data manipulation and a
separate vignette, “algo", has been written to give algorithmic details
of analyses introduced with *poppr*.

As of spring 2014, Drs. Niklaus J. Grünwald, Sydney E. Everhart, and I
have co-authored a primer on using R for population genetic analysis. It
is located at <http://grunwaldlab.github.io//Population_Genetics_in_R>
and the source code can be found [on our github
site](https://github.com/grunwaldlab/Population_Genetics_in_R).
...
the *poppr* forum at
<http://groups.google.com/group/poppr>. You can submit bug reports there
or on our github site: <https://github.com/grunwaldlab/poppr>
...
`  `
    ...
    ## poppr version 2.2.0.99-32 and with the following citation:
    ## 
...
All dependencies (*adegenet, pegas, vegan, ggplot2, phangorn, ape,
reshape2* and *igraph*) will also be installed. In the unfortunate case
this does not work, consult
<http://cran.r-project.org/doc/manuals/R-admin.html#Installing-packages>.

### From GitHub

GitHub is a repository where you can find all stable and development
versions of *poppr*.

Since *poppr* contains C code, it needs to be compiled,
which means that you need a working C compiler. If you are on Linux, you
should have that, but if you are on Windows or OSX, you might need to
download some special tools:
...
If you are the adventurous type and are willing to test out unreleased
versions of the package, you can also install the development version:

Users who install this version do so at their own risk. Since it is a
development version, documentation may be incomplete or non-existent for
new functions.

### From Source

The tarball for *poppr* can be downloaded from CRAN:
<http://cran.r-project.org/package=poppr> or via github at
<https://github.com/grunwaldlab/poppr>.

You will need a C compiler to install this way. See the section above
for details.

If you choose to install *poppr* from a source file, you
should first make sure to install all of the dependencies with the
following command:

`       `\
`             `\

After installing dependencies, download the package to your computer and
...
Importing a file into R involves you knowing the path to your file and
then typing that into R’s console. `getfile()` will help provide a point
and click interface for selecting a file. This is simply a tool to help
you get started. As you get better with R, you might feel that you don’t
need it at all.
...
data-label="getfile window"></span>](getfile)
...
We will use `x$files` to access the file. The file is in the <span>
<span style="font-variant:small-caps;"></span></span> format, so we will
import it using <span> ``</span> and then analyze it with the function
...
look at the data.
    ...
    ## 2      Athena_2  12  12 10.00   NaN 2.485 12.00  0.917 1.000 0.1999  4.16 0.128 myData
...
“object”, which is stored in the computer’s memory. Because of this,
it’s important to know the difference between a **file** and an
**object**. When anyone talking about importing a **file** into R, they
are referring to a spreadsheet or text file that lives in a folder on
your hard drive. Spreadsheet files (saved as \*.csv files) are normally
imported through the R function `read.table()`. The output of
`read.table()` is a `data.frame`. A `data.frame` is an **object**
represented in your computer’s memory. This means that it only exists
for as long as R is running.

The good thing about having objects stored in memory is that you can
manipulate them in any way and not affect the source of those data.
Since R is a scripted language (instead of point-and-click), any of your
manipulations can be saved in a separate R file that can be easily
adapted to new data. Of course, most data are not going to be entered
into R manually. Usually they will be formatted in a manner that can be
read by popular population genetics programs.

As previously mentioned, since *poppr* is based on
*adegenet*, it’s possible to read in the following file
formats into a `genind` object with the function `import2genind()`:

-   <span><span style="font-variant:small-caps;">Fstat</span></span>

-   <span><span style="font-variant:small-caps;">Genepop</span></span>

-   <span><span style="font-variant:small-caps;">Genetix</span></span>

-   <span><span style="font-variant:small-caps;">Structure</span></span>

Here, we introduce a new way of importing data into a `genind` or
`genclone` object from a <span
style="font-variant:small-caps;">GenAlEx</span> formatted file.
...
[@Peakall:2012; @Peakall:2006]. GenAlEx runs within the Excel
environment and can be very powerful in its analyses.
*Poppr* has added the ability to read \*.CSV files[^2]
produced in the GenAlEx format. It can handle data types containing
regions and geographic coordinates, but currently cannot import allelic
frequency data from GenAlEx. Using the *poppr* function
`read.genalex` will import your data into *adegenet*’s
`genind` object or *poppr*’s `genclone` object (more
information on that below). For ways of formatting a GenAlEx file, see
the manual here:
...
Breed contained within the `@other` slot (detailed in <span> <span
style="font-variant:small-caps;"></span></span>). We will first set
...
and their relative sizes.<span
data-label="microbov unmodified"></span>](unmod_dat)
...
population factor to use as hierarchical levels in a <span> <span
style="font-variant:small-caps;"></span></span>
...
Highlighted in red in figure \[microbov unmodified\] are definitions of
...
![The first 15 individuals and 4 loci of the microbov data set. This is
the same figure as above, however the populations and counts have been
removed from the header row and the third number in the header has been
replaced by 1.<span data-label="microbov modified"></span>](mod_dat)
...
-   `geo -` GenAlEx allows you to have geographic data within your file.
    ...
    be explained later in <span> <span
    style="font-variant:small-caps;"></span></span>)
...
-   `genclone -` This flag will convert your data into a `genclone`
    object (see <span> <span
    style="font-variant:small-caps;"></span></span> for more info).
...
-   `recode -` If your data is polyploid data, this gives you the option
    to recode it. (See <span> <span
    style="font-variant:small-caps;"></span></span> for details)
...
We have a short example of <span
style="font-variant:small-caps;">GenAlEx</span> formatted data with no
...
can be used with any function in *poppr* and
*adegenet*
...
*Adegenet* already supports the import of <span
style="font-variant:small-caps;">FSTAT</span>, <span
style="font-variant:small-caps;">Structure</span>, <span
style="font-variant:small-caps;">Genpop</span>, and <span
style="font-variant:small-caps;">Genetix</span> formatted files, so if
...
> Default Command:
>
>     genind2genalex(gid, filename = "genalex.csv", quiet = FALSE, 
...
First, a simple example for the rootrot data we demonstrated in section
1.4.2:
...
Here’s an example of exporting the nancycats data set into <span
style="font-variant:small-caps;">GenAlEx</span> format with geographic
information. If we look at the nancycats geographic information, we can
see it’s coordinates for each population, but not each individual:
...
![image](figure/ex_data_picture-1){width="7in" height="2in"}

When we look at the data derived from table \[example\_table\], we see
...
At each locus, the allele counts for each individual sum to the ploidy,
$p$. Homozygotes are denoted as having an allele count of $p$ at a
single allele within a locus, while heterozygotes have their allele
counts represented as $<
p$ wher $p$ = ploidy across multiple alleles in a single locus. Along
with this matrix, are elements that define the names of the individuals,
loci, alleles, and populations. If you wish to know more, see the
*adegenet* “Getting Started” manual.
...
Let’s show an example of a `genclone` object. First, we will take an
existing `genind` object and convert it using the function `as.genclone`
(We can also use the function <span> ``</span> to import as `genclone`
or `genind` objects). We will use the `Aeut` data set because it is a
clonal data set that has a simple population strata [@Grunwald:2006].
The data set is here: <http://dx.doi.org/10.6084/m9.figshare.877104> and
it is AFLP data of the root rot pathogen *Aphanomyces euteiches*
...
represent the subpopulation. Let’s take a look at what the `genind`
object looks like:
...
“population\_hierarchy". This is left over from *poppr*
version 1.x behavior. Since the `genind` object now has a `@strata`
slot, we can use it to set the stratification (previously called
“hierarchy”).
...
We can see here that it shows less information, but it gives us a very
simple overview of our data. Don’t be fooled, however, because it
contains the same information as a `genind` object.
...
![image](figure/pinf_table-1){width="0.5\linewidth"}
...
$\times$ 3 alleles). The first allele at each locus is `000`. Let’s take
...
This method allows <span style="font-variant:small-caps;">Bruvo’s
Distance</span> [@Bruvo:2004] and the <span
style="font-variant:small-caps;">Index of Association</span>
...
This argument will retain missing data in your data set. It’s useful for
functions that utilize `missingno` internally, such as `mlg.filter()`,
`poppr.amova()`, or `aboot()`.

This replaces missing data with the mean allele frequencies in the
entire data set .
...
![image](figure/initializing_poppr-1){width=".8\linewidth"}
...
function <span> ``</span>, we will zoom into the first five individuals
at the first locus.

` `

    ##      fca8.133 fca8.135 fca8.137 fca8.139 fca8.141 fca8.143
...
` `
    ...
    ##      fca23.144 fca23.146 fca23.148 fca23.150 fca43.133 fca43.135
    ## N215         0         1         0         0         0         0
    ## N216         0         2         0         0         0         0
    ## N217         0         1         0         0         0         0
    ## N218         0         0         0         0         0         0
...
-   `mat -` (see section <span> <span
    style="font-variant:small-caps;"></span></span> for
    more information) A matrix produced from the <span>
    ``</span> function. This overrides the *pop* argument and subsets
    this table instead.
...
euteichies* data that we used in the <span> <span
style="font-variant:small-caps;"></span></span> since that data set is
known to include clonal populations [@Grunwald:2006]. Try playing around
with the data and see what different combinations of the `hier`, and
...
When you read in data with <span> ``</span>, the default is to represent
...
-   `method -` a number indicating the method of sampling you wish
    ...
    1.  **Permute Alleles (default)** This is a sampling scheme that
        will **permute alleles within the locus.** The example above
        might become tables \[tab:permsinglebest\]
        and \[tab:permtwobest\].
    ...
    2.  **Parametric Bootstrap** The previous scheme reshuffled the
        ...
        frequencies as weights.** Tables \[tab:param1\] and
        \[tab:param2\] are examples of what I mean.
    ...
    3.  **Non-Parametric Bootstrap** The third method is sampling with
        ...
        but with no assumption about the allele frequencies** (tables
        \[tab:nonparam1\] and \[tab:nonparam2\]).
    ...
    4.  **Multilocus permutation** This is called Multilocus permutation
        ...
        Using our example above, tables \[tab:singlebest\] and
        \[tab:twobest\] are shuffled with method 4.
...
![image](figure/bd_histogram-1){width="0.5\linewidth"}
...
<span> <span style="font-variant:small-caps;"></span></span>. We will
only retain loci that have a minor allele frequency of $\geq 5\%$
...
and across populations in <span> <span
style="font-variant:small-caps;"></span></span> objects from the
*adegenet* package. We will be using the H3N2 data set
containing SNP data from isolates of the H3N2 virus from 2002 to 2006.
Note that genclone objects are optimal for these analyses. For a more
...
Counting the number of MLGs in a population is the first step for these
analyses as they allow us to see how many clones exist. With the <span>
<span style="font-variant:small-caps;"></span></span> object, This
...
-   `sublist -` Populations to include (Defaults to “ALL”). see <span>
    ``</span>

-   `blacklist -` Populations to exclude. see <span> ``</span>

-   `mlgsub -` see <span> ``</span> Only analyze specified MLGs. The
...
>         mlgsub = NULL, bar = TRUE, plot = TRUE, total = FALSE, quiet = FALSE)
...
![An example of a bar-chart produced by `mlg.table`. Note that this data
set would produce several such charts but only the chart for Norway is
shown here.<span
data-label="nortable"></span>](figure/mlgbarplot-1){width="0.8\linewidth"}
...
` `
    ...
    ##   6  17  39  42  45  51  60  72  73  90 108 123 129 134 145 148 149 157 168 171 177 225 
...
![An example of a rarefaction curve produced using a MLG table.<span
data-label="rarecurve"></span>](figure/mlgrareplot-1){width="0.5\linewidth"}
...
set. One thing that the <span> <span
style="font-variant:small-caps;"></span></span> object keeps track of is
...
The integers produced are the MLG assigment of each individual in the
...
\[mlg:mix:mlg.vector\] We will take `UGNN` (MLGs crossing UK, Germany,
...
We can also use the vector of MLGs to subset <span> ``</span> with the
`mlgsub` flag.
...
![All populations that contain these MLGs<span
data-label="nortable2"></span>](figure/mlgsub_flagshow-1){width="0.8\linewidth"}
...
![image](figure/Aeut_MLG_Aeut-1){width="0.8\linewidth"}
...
![image](figure/Aeut_MLG_title-1){width="0.8\linewidth"}
...
![image](figure/Aeut_MLG_theme-1){width="0.8\linewidth"}
...
![image](figure/Aeut_MLG_theme_axis-1){width="0.8\linewidth"}

And, if for some bizzarre reason, you liked the color gradient in
*poppr* version 1, you can get that back by adding the
*fill* aesthetic:

`     `

![image](figure/Aeut_silly-1){width="0.8\linewidth"}
...
the data in the ggplot2 object and plot the data yourself:
    ...
    ## # A tibble: 6 x 4
    ##   Population    MLG count    fac
    ##       <fctr> <fctr> <int> <fctr>
    ## 1     Athena  MLG.1     1    120
    ## 2 Mt. Vernon  MLG.3     1    119
    ## 3 Mt. Vernon  MLG.4     1    118
    ## 4 Mt. Vernon  MLG.5     1    117
    ## 5 Mt. Vernon  MLG.6     1    116
    ## 6     Athena  MLG.7     1    115
...
-   Bitmap based editors (for jpeg, bmp, png, etc...)
    ...
    > -   \[ <span style="font-variant:small-caps;">The GIMP \]
    >     Free, cross-platform. <http://www.gimp.org></span>
    >
    > -   \[ <span style="font-variant:small-caps;">Paint.net \] Free,
    >     Windows only. <http://www.getpaint.net></span>
    >
    > -   \[ <span style="font-variant:small-caps;">Adobe Photoshop \]
    >     Pricey, Windows and Mac.
    >     <http://www.adobe.com/products/photoshop.html></span>
    >
-   Scalable Vector Graphics based editors (for svg, pdf)

    > -   \[ <span style="font-variant:small-caps;">Inkscape \] Free,
    >     cross-platform <http://inkscape.org></span>
    >
    > -   \[ <span style="font-variant:small-caps;">Adobe Illustrator \]
    >     Pricey, Windows and Mac.
    >     <http://www.adobe.com/products/illustrator.html></span>
    >
