...
out that you have 5 **contracted** multilocus genotypes, but you have some cryptic code underdeath. This is shorthand for the threshold **[t]** and distance **[d]** you used to filter the multilocus genotypes. This indicates that your object remembers what object was used for the distance. This means that you don't always have to specify the distance when assigning a threshold:
...
> DANGER! DANGER! While this is a convenient function, if you use a matrix or function that was created by you, you **must not delete it**. The reason being that if you delete the matrix used to inform your filtering, you will get an error:
...
function available. We will use the `raw_dist()` function that we defined earlier.
...
The safest way, perhaps is to use a function defined in *poppr*. For example, Bruvo's distance `r citep(bib["Bruvo:2004"])`.
...
This can give you estimates of diversity when all samples are of equal size.
...
results ever so slightly. As an example, let's look at the `monpop` filtered
...
does not mean that the procedure doesn't work, it just means that one must be
...
mutations/errors/missing data. As with all analyses, it is important to take
