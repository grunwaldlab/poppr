...
\hlcom{# Adding Bruvo's distance at the end because we need to specify repeat}
\hlcom{# length.}
...
AMOVA in \poppr{} acts as a wrapper for the \textit{ade4} implementation, which is
an implementation of Excoffier's original formulation \cite{excoffier1992analysis}.
As the calculation relies on a genetic distance matrix, \poppr{} calculates the 
distance matrix as the number of differing sites between two genotypes. This is
equivalent to Provesti's distance multiplied by the ploidy and number of loci. 
This is also equivalent to Kronecker's delta as presented in \cite{excoffier1992analysis}. 
...
\subsection{Rarefaction, Shannon - Wiener, Stoddard and Taylor indices}
...
Where $1/\lambda$ is Stoddard and Taylor's index and $H$ is Shannon diversity.
...
genotypes observed and $N$ is the sample size. Essentially, it's a correction
for Simpson's index by sample size.
