...
test_that("aboot can utilize anonymous functions", {
  ...
  nantree <- aboot(
    nan9,
    dist = function(x) dist(x$tab, method = "manhattan"),
    sample = 20,
    quiet = TRUE
  )
...
test_that("aboot can utilize bootgen2genind", {
  ...
  suppressWarnings(
    nantreebg <- aboot(
      nan9,
      dist = function(x) nei.dist(bootgen2genind(x)),
      sample = 20,
      quiet = TRUE
    )
  )
  suppressWarnings(
    nantree <- aboot(nan9, dist = "nei.dist", sample = 20, quiet = TRUE)
  )
...
test_that("aboot can handle populations", {
  ...
  set.seed(999)
  expect_warning(
    Atreen <- aboot(Aeut.pop, tree = "nj", sample = 20, quiet = TRUE),
    "Felsenstein"
  )
  Atreeu <- aboot(Aeut.pop, sample = 20, quiet = TRUE)
  expect_warning(
    AtreeF <- aboot(Aeut.pop, tree = myFastME, sample = 20, quiet = TRUE),
    "Felsenstein"
  )
