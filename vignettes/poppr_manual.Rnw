...
\texttt{diss.dist} & Absolute genetic distance (see \texttt{prevosti.dist}) \\
...
\texttt{prevosti.dist} & Prevosti's absolute genetic distance \\
%----------------|
\hline \\[-1.5ex]
...
\texttt{mll.reset} & Reset multilocus lineages.  \\
...
\texttt{rrmlg} & obtain round-robin estimates of multilocus genotypes. \\
\texttt{rraf} & obtain round-robin estimates of allele frequencies. \\
\texttt{pgen} & Genotype probability, $p_{gen}$. \\
\texttt{psex} & Probability of observing a genotype more than once, $p_{sex}$. \\
%----------------|
\hline \\[-1.5ex]
