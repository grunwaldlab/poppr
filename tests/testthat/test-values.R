...
test_that("Bruvo between places distances in the same location has bruvo distance", {
  ...
  n3 <- nancycats[pop = 3]
  n4 <- nancycats[pop = 4]
  btwn <- poppr:::bruvo.between(
    n3[1:3],
    n4[1:5],
    replen = rep(2, 9),
    by_locus = TRUE
  )
  dist <- poppr:::bruvo.dist(
    repool(n3[1:3], n4[1:5]),
    replen = rep(2, 9),
    by_locus = TRUE
  )
  comparisons <- c(
...
test_that("Bruvo's distance will throw an error if there are not enough named lengths", {
  ...
  expect_error(
    bruvo.dist(p10, replen = other(p10)$REPLEN),
    "repeat lengths... WHAAA"
  )
})
...
test_that("Internal function fix_negative_branch works as expected.", {
  ...
  # order.
  expect_equivalent(
    min(fix_tree$edge.length - the_tree$edge.length),
    min(the_tree$edge.length)
  )
})
...
test_that("diversity_stats returns expected values", {
  ...
  expected <- structure(
    ...
    .Dimnames = structure(
      list(
        Pop = c("Athena", "Mt. Vernon", "Total"),
        Index = c("H", "G", "simp", "E.5")
      ),
      .Names = c("Pop", "Index")
    )
  )
...
test_that("ia and pair.ia return same values", {
  ...
  pair_posi <- grepl(locpair[1], rownames(pc_pair)) &
    grepl(locpair[2], rownames(pc_pair))
