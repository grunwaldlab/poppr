...
Let \textbf{A} a table containing allelic frequencies with $t$ populations (rows) and $m$ alleles (columns).\\
...
\begin{equation}
  a_{ij}^k\\ (1 \leq i \leq t,\\ 1 \leq j \leq \nu,\\ 1 \leq k \leq m(j))
\end{equation}

the value of the initial table.

\begin{equation}
  a_{ij}^{\cdot}=\sum_{k=1}^{m(j)}a_{ij}^k 
\end{equation}

and 

\begin{equation}
  p_{ij}^k=\frac{a_{ij}^k}{a_{ij}^{\cdot}}
\end{equation}


Let \textbf{P} the table of general term $p_{ij}^k$


The sum of all allele frequencies for a single population at a single locus is one.
\begin{equation}
 p_{ij}^{\cdot}=\sum_{k=1}^{m(j)}p_{ij}^k=1
\end{equation}
...
The the sum of the entire table is the sum of all loci multiplied by the number of populations. 
\begin{equation}
...
Absolute Genetic distance

... are all the same! Note that these two really are exactly the same. They are an extension of Bray-Curtis Dissimilarity.
...
This is based off of a specific model where it is necessary to specify the repeat lengths of the markers at each locus. Eg a repeat of $(CAT)^n$ would have a repeat length of 3. 
