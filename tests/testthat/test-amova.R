...
test_that("AMOVA can calculate within individual variance for diploids", {
  ...
  expect_output(
    poppr.amova(mics, ~breed, correction = "cai"),
    "Cailliez constant"
  )
  expect_output(
    poppr.amova(mics, ~breed, correction = "lin"),
    "Lingoes constant"
  )
  expect_error(
    poppr.amova(mics, ~breed, correction = "wha", quiet = TRUE),
    "cailliez"
  )
  expect_message(
    micwithout <- poppr.amova(mics, ~breed, within = FALSE),
    "Removing"
  )
...
test_that("AMOVA will not calculate within-individual variance for mixed ploids", {
  ...
  rho <- c(0.688374795330904, 0.445443116797669, 0.438064490572017)
  expect_warning(
    res <- poppr.amova(pg, ~ group / population, within = TRUE),
    wrn
  )
...
test_that("AMOVA will give an extra warning for polyploids with zeroes", {
  ...
  expect_warning(
    res <- poppr.amova(
      recode_polyploids(pg, addzero = TRUE),
      ~ group / population,
      within = TRUE
    ),
    wrn
