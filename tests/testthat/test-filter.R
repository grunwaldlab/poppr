...
grid_example <- matrix(c(1, 1, 5, 9, 9, 4, 1, 1, 1, 4), ncol = 2)
...
test_that("internal filtering will throw an error for negative distances", {
  ...
  xdn[1] <- -3
  expect_error(
    mlg.filter(x, distance = xdn, threshold = 4.51),
    "Distance matrix must not contain negative distances"
  )
  xdn[1] <- -0.3
  expect_warning(
    .Call("neighbor_clustering", as.matrix(xdn), mll(x), 4.51, "f", 1L),
    "The data resulted in a negative or invalid distance or cluster id"
  )
})
...
test_that("Infinite distances will produce an error", {
  ...
  expect_error(
    mlg.filter(x, distance = xdn, threshold = 4.51),
    "Data set contains missing or invalid distances"
  )
})
...
test_that("mlg.filter errors when distance matrix contains missing data", {
  skip_on_cran()
  expect_error(
    mlg.filter(x20160810_mon20, distance = nei.dist) <- 0,
...
test_that("mlg.filter errors when the distance matrix is not of equal size", {
  skip_on_cran()
  expect_error(
    mlg.filter(monpop, distance = diss.dist(x20160810_mon20)) <- 0,
    "distance matrix \\(20\\)"
  )
  expect_error(
    mlg.filter(monpop, distance = x20160810_neifun) <- 0,
...
test_that("mlg.filter throws a warning when attempting to use 'mean' with diss.dist", {
  skip_on_cran()
  expect_warning(
    mlg.filter(x20160810_mon20, distance = diss.dist, missing = "mean") <- 0
  )
})
