...
% The first page will have the title, abstract, and then the Poppr logo at the bottom.
\maketitle
\begin{abstract}
...
<<echo=FALSE>>=
library(poppr)
x <- list(files="rootrot.csv", path="/path/to/R/poppr/files")
@
...
poppr.all(c(system.file("files/rootrot.csv", package="poppr"), system.file("files/simulated.dat", package="poppr")))
@
...
list(
  files = dir(dirname(system.file("files/nancycats.gtx",package="adegenet"))),
...
list(
  files = list.files(dirname(system.file("files/nancycats.gtx",package="adegenet")), pattern="nancy"),
  path = "/path/to/R/adegenet/files"
)
@
...
list(
  files = list.files(dirname(system.file("files/nancycats.gtx",package="adegenet")), pattern="gtx"),
  path = "/path/to/R/adegenet/files"
)
@
...
list(
  files = list.files(dirname(system.file("files/nancycats.gtx",package="adegenet")), pattern="dat"),
  path = "/path/to/R/adegenet/files"
)
@
...
\tab\tab The index of association has previously been implemented in a couple of programs including \textit{multilocus} \cite{Agapow:2001} and \textit{LIAN} \cite{Haubold:2000}, but these are programs that are no longer well supported and can be difficult to install and do not run in batch mode.
...
set.seed(1001)
ia(popsub(nancycats, 5), sample=999, hist=FALSE)
@
...
set.seed(1001)
nan9tree <- bruvo.boot(popsub(nancycats, 8:9), replen=rep(1,9), sample=1000, quiet=TRUE, cutoff=50)
@
\end{figure}
...
\centering
\caption{\footnotesize Default for \texttt{greywidth()}, weighted for small values.}
...
\tab\tab Use this function to draw a minimum spanning network from your data set and a distance matrix derived from your data set. Since there are hundreds of distances that can be calculated for genetic data, and since I want to be able to graduate at some point in this decade, functions to automatically calculate distances and draw the minimum spanning networks will be few and far between.

This function is an attempt to meet the user halfway and draw a minimum spanning network provided that the user has supplied two things:
\begin{enumerate}
...
\begin{itemize}
  ...
  \end{description}
  \item \emph{missingno functions:} See Section 2.1 \\ Note that all analyses in this function ignore/impute missing data by default.
    \begin{description}
  ...
  \begin{description}
    ...
    \item[keep -] A vector of integers as indexes for the \texttt{hier} flag indicating which levels of the hierarchy you want to analyze. See section 2.4 for details. 
  \end{description}
...
set.seed(2004)
poppr(system.file("files/simulated.dat", package="poppr"), sample=999, method=1, quiet=TRUE, hist=FALSE)
@
\begin{figure}[h!]
  \centering
  \caption{\footnotesize Output of multilocus-style sampling.}
...
Of course, Agapow and Burt showed that this index increases steadily with the number of loci, so they came up with an approximation that is widely used, $\bar r_d$ \cite{Agapow:2001}. For the derivation, see the manual for \textit{multilocus}. The equation is as follows, utilizing equations (4), (5), and (6) \cite{Agapow:2001}:
\beq
...
\tab\tab R has the ability to produce nice graphics from most any type of data, but to get these graphics into a report, presentation, or manuscript can be a bit challenging. It's no secret that the R Documentation pages are a little difficult to interpret, so I will give the reader here a short example on how to export graphics from R. Note that I will only present code here and will refer you to other places in the vignette for the images. 

I strongly encourage the user to use the GUI Rstudio \url{http://www.rstudio.com/}

\subsubsection{Basics}
\tab\tab Before you export graphics, you have to ask yourself what they will be used for. Many people do not realize that there is a large difference between what is appropriate to display on a website, what is appropriate for a presentation, and what is appropriate to print in a publication or poster. Here, I will give some general guidelines for graphics (note that these are merely suggestions, not defined rules).
\begin{enumerate}
  \item \textbf{What you see is not always what you get} I will reiterate this because I feel it is important. I have often seen presentations where the colors were too light or posters with painfully pixellated graphs. Think about what you are going to be using a graphic for and how it will appear to the intended audience given the media type.
  \item \textbf{$\geq$ 300 dpi or die} For any sort of printed material that requires a raster based image, 300dpi (dots per inch) is the absolute minimum resolution you should use. For simple black and white line images, 1200dpi is better. This will leave you with crisp, professional looking images.
  \item \textbf{Save as SVG, then rasterize} Raster images (bmp, png, jpg, etc...) are based off of the number of pixels or dots per inch it takes to render the image. This means that the raster image is more or less a very fine mosaic. Vector images (svg) are built upon several interconnected polygons, arcs, and lines that scale relative to one another to create your graphic. With vector graphics, you can produce a plot and scale it to the size of a building if you wanted to. When you save to an SVG first, you can also manipulate it in programs such as Adobe Illustrator or Inkscape. 
...
\tab\tab Here is a short list of image editors (both free and for a price) that you can use to edit your graphics.
\begin{itemize}
  \item Bitmap based editors
  \begin{itemize}
    \item[ The GIMP ] Free, cross-platform. \url{http://www.gimp.org}
    \item[ Paint.net ] Free, Windows only. \url{http://www.getpaint.net}
    \item[ Adobe Photoshop ] Pricey, Windows and Mac. \url{http://www.adobe.com/products/photoshop.html}
  \end{itemize}
  \item Scalable Vector Graphics based editors
  \begin{itemize}
    \item[ Inkscape ] Free, cross-platform \url{http://inkscape.org}
    \item[ Adobe Illustrator ] Pricey, Windows and Mac. \url{http://www.adobe.com/products/illustrator.html}
  \end{itemize}
\end{itemize}
...
\item missingno(pop, type = "loci", cutoff = 0.05, quiet = FALSE)
...
\item clonecorrect(pop, hier = c(1), dfname = "population\_hierarchy", combine = FALSE, keep = 1)
...
\item bruvo.boot(pop, replen = c(2), B = 100, tree = "upgma", showtree = TRUE, cutoff = NULL, quiet = FALSE)
\item greycurve(glim = c(0, 0.8), gadj = 3, gweight = 1)
\item bruvo.msn(pop, replen = c(1), palette = topo.colors, sublist = "All", blacklist = NULL, vertex.label = "MLG", gscale = TRUE, glim = c(0, 0.8), gadj = 3, gweight = 1, wscale = TRUE, ...)
poppr.msn(pop, distmat, palette = topo.colors, sublist = "All", blacklist = NULL, vertex.label = "MLG", gscale = TRUE, glim = c(0, 0.8), gadj = 3, gweight = 1, wscale = TRUE, ...)
\item poppr(pop, total = TRUE, sublist = c("ALL"), blacklist = c(NULL), sample = 0, method = 1, missing = "ignore", cutoff=0.05, quiet = "minimal", clonecorrect = FALSE, hier = c(1), keep = 1, dfname = "population\_hierarchy", hist = TRUE, minsamp = 10)
\end{itemize}
