...
test_that("test_replen and fix_replen work for larger length vectors", {
  ...
  expect_warning(
    expect_error(
      test_replen(nancycats, bad_named_nanrep11),
      "repeat lengths... bob, foo, bar"
    ),
    "bob"
  )
...
test_that("test_replen and fix_replen will not work for short vectors", {
  ...
  expect_error(
    test_replen(nancycats, named_nanrep[1:7]),
    "length of repeats \\(7\\)"
  )
})
...
test_that("fix_replen throws errors for weird replens", {
  ...
  data(partial_clone)
  expect_warning(
    fix_replen(partial_clone, rep(10, 10)),
    paste(locNames(partial_clone), collapse = ", ")
  )
  expect_warning(
    fix_replen(partial_clone, rep(10, 10), fix_some = FALSE),
    "Original repeat lengths are being returned"
  )
  expect_warning(
    fix_replen(partial_clone, rep(2, 10)),
    "The repeat lengths for Locus_2, Locus_7, Locus_9 are not consistent."
  )
  expect_warning(
    fix_replen(partial_clone, rep(2, 10)),
    "Repeat lengths with some modification are being returned: Locus_3"
  )
})
