...
test_that("recode_polyploids won't take diploid data", {
  ...
  expect_warning(
    recode_polyploids(nancycats),
    "Input is not a polyploid data set, returning original."
  )
})
...
test_that("recode_polyploids will go there and back again", {
  ...
  expect_warning(
    pr <- recode_polyploids(Pinf, addzero = TRUE),
    "addzero = TRUE"
  )
...
test_that("haplodiploids will work", {
  skip_on_cran()
  df <- matrix(c("0/2", "2/1", "3/0", "0/1", "2/2", "1/1"), nrow = 3)
  x <- df2genind(df, sep = "/")
