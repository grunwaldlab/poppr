...
pc_comparison <- structure(
  list(
    ...
    Hexp = c(
      ...
      0.505072463768116,
      0.559212121212121
    ), #c(0.849679487179487, 0.83629191321499, 0.714756944444444, 0.750347222222222, 0.78384),
    Ia = c(
      2.1580763424628,
      1.87492360969648,
      1.15572679509632,
...
test_that("poppr perform clone correction", {
  ...
  expect_identical(
    res_ps2[-c(1, length(res_na))],
    res_naf[-c(1, length(res_naf))]
  )
...
test_that("poppr will correct missing values as mean for P/A markers", {
  ...
  expect_warning(
    pm <- poppr(A, missing = "mean", quiet = TRUE, total = FALSE),
    "adegenet_2.0-0"
  )
...
test_that("poppr skips over sample sizes less than three", {
  skip_on_cran()
  expect_warning(
    plt <- poppr(partial_clone[1:8], sample = 10, quiet = TRUE),
...
test_that("poppr.all works on list of files", {
  skip_on_cran()
  expect_output(
    out <- poppr.all(list(afile, pc = partial_clone)),
    " | File: rootrot.csv"
  )
})
