...
## Population information:
## 
##      17 population hierarchies - accession length host ... lon lat month
##       0 populations defined.
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.933, 0.933, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{addhierarchy}\hlstd{(virus, virus_info,} \hlkwc{name} \hlstd{=} \hlstr{"month"}\hlstd{)}  \hlcom{# By Argument}
\end{alltt}
\begin{verbatim}
## 
## This is a genclone object
...
\end{verbatim}
\begin{alltt}
\hlkwd{addhierarchy}\hlstd{(virus)} \hlkwb{<-} \hlstd{virus_info}  \hlcom{# By Assignment}
\hlstd{virus}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.933, 0.933, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{namehierarchy}\hlstd{(virus)} \hlkwb{<-} \hlopt{~}\hlstd{country}\hlopt{/}\hlstd{year}\hlopt{/}\hlstd{month}
\hlstd{virus}
\end{alltt}
\begin{verbatim}
## 
...
## 
##      2 population hierarchies - field sample
##     10 populations defined - 1 2 3 ... 8 9 10
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\begin{verbatim}
##   MLG.3   MLG.9  MLG.31  MLG.75  MLG.80  MLG.86  MLG.95  MLG.97 MLG.104 MLG.110 
##       2       4       2       8       2       2       2       4       2       3
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlkwd{str}\hlstd{(v.vec)}  \hlcom{# Analyze the structure.}
\end{alltt}
\begin{verbatim}
##  int [1:1903] 605 605 672 675 674 673 670 671 670 678 ...
\end{verbatim}
\begin{alltt}
\hlkwd{length}\hlstd{(}\hlkwd{unique}\hlstd{(v.vec))}  \hlcom{# count the number of MLGs}
\end{alltt}
\begin{verbatim}
## [1] 752
\end{verbatim}
\begin{alltt}
\hlstd{virus}  \hlcom{# equal to the first number in this output.}
\end{alltt}
\begin{verbatim}
## 
## This is a genclone object
...
that the first two individuals are both designated as MLG 605,
...
\definecolor{shadecolor}{rgb}{0.933, 0.933, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{UGNN}  \hlcom{# Show what we are looking for}
\end{alltt}
\begin{verbatim}
## MLG.315 MLG.317 MLG.620 
##     315     317     620
\end{verbatim}
\begin{alltt}
\hlstd{UGNN_match} \hlkwb{<-} \hlstd{v.vec} \hlopt{%in%} \hlstd{UGNN}
\hlkwd{table}\hlstd{(UGNN_match)}  \hlcom{# How many individuals matched to those three MLGs?}
\end{alltt}
\begin{verbatim}
## UGNN_match
## FALSE  TRUE 
...
\definecolor{shadecolor}{rgb}{0.933, 0.933, 0.933}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{indNames}\hlstd{(virus)[UGNN_match]}
\end{alltt}
\begin{verbatim}
...
{\centering \includegraphics[width=0.5\linewidth]{figure/unnamed-chunk-5} 
...
\begin{tabular}{llll}
  \hline
 Method & Function & Euclidean \\ 
  \hline
\seclink{distance:dissimilarity}{Provesti} & \texttt{provesti.dist} & No \\
 & \texttt{diss.dist} & \\
\seclink{distance:nei}{Nei} & \texttt{nei.dist} & No \\
\seclink{distance:edwards}{Edwards} & \texttt{edward.dist} & Yes \\
\seclink{distance:reynolds}{Reynolds} & \texttt{reynold.dist} & Yes \\
\seclink{distance:rogers}{Rogers} & \texttt{rogers.dist} & Yes \\
\seclink{distance:bruvo}{Bruvo} & \texttt{bruvo.dist} & No \\
   \hline
\end{tabular}
\end{table}
...
\begin{alltt}
\hlkwd{data}\hlstd{(nancycats)}
\hlstd{nancy9} \hlkwb{<-} \hlkwd{popsub}\hlstd{(nancycats,} \hlnum{9}\hlstd{)}
\hlstd{repeats} \hlkwb{<-} \hlkwd{rep}\hlstd{(}\hlnum{2}\hlstd{,} \hlnum{9}\hlstd{)}
\hlstd{pro_nan} \hlkwb{<-} \hlkwd{provesti.dist}\hlstd{(nancy9)}
\hlstd{nei_nan} \hlkwb{<-} \hlkwd{nei.dist}\hlstd{(nancy9)}
\hlstd{edw_nan} \hlkwb{<-} \hlkwd{edwards.dist}\hlstd{(nancy9)}
\hlstd{rey_nan} \hlkwb{<-} \hlkwd{reynolds.dist}\hlstd{(nancy9)}
\hlstd{rog_nan} \hlkwb{<-} \hlkwd{rogers.dist}\hlstd{(nancy9)}
\hlstd{bru_nan} \hlkwb{<-} \hlkwd{bruvo.dist}\hlstd{(nancy9,} \hlkwc{replen} \hlstd{= repeats)}
\hlstd{distlist} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlkwc{Provesti} \hlstd{= pro_nan,} \hlkwc{Nei} \hlstd{= nei_nan,} \hlkwc{Edwards} \hlstd{= edw_nan,} \hlkwc{Reynolds} \hlstd{= rey_nan,}
    \hlkwc{Rogers} \hlstd{= rog_nan,} \hlkwc{Bruvo} \hlstd{= bru_nan)}
\hlstd{combs} \hlkwb{<-} \hlkwd{combn}\hlstd{(}\hlkwc{x} \hlstd{=} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(distlist),} \hlkwc{m} \hlstd{=} \hlnum{2}\hlstd{)}
\hlstd{mtest} \hlkwb{<-} \hlkwd{apply}\hlstd{(combs,} \hlnum{2}\hlstd{,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{mantel.randtest}\hlstd{(distlist[[x[}\hlnum{1}\hlstd{]]], distlist[[x[}\hlnum{2}\hlstd{]]])}\hlopt{$}\hlstd{obs)}
\hlkwd{attr}\hlstd{(mtest,} \hlstr{"class"}\hlstd{)} \hlkwb{<-} \hlstr{"dist"}
\hlkwd{attr}\hlstd{(mtest,} \hlstr{"Size"}\hlstd{)} \hlkwb{<-} \hlnum{6}
\hlkwd{attr}\hlstd{(mtest,} \hlstr{"Labels"}\hlstd{)} \hlkwb{<-} \hlkwd{names}\hlstd{(distlist)}
\hlstd{mtest} \hlkwb{<-} \hlkwd{as.matrix}\hlstd{(mtest)}
\hlkwd{diag}\hlstd{(mtest)} \hlkwb{<-} \hlnum{1}
\hlkwd{library}\hlstd{(reshape2)}
\hlkwd{library}\hlstd{(ggplot2)}
\hlkwd{ggplot}\hlstd{(}\hlkwd{melt}\hlstd{(mtest))} \hlopt{+} \hlkwd{geom_tile}\hlstd{(}\hlkwd{aes_string}\hlstd{(}\hlkwc{x} \hlstd{=} \hlstr{"Var1"}\hlstd{,} \hlkwc{y} \hlstd{=} \hlstr{"Var2"}\hlstd{,} \hlkwc{fill} \hlstd{=} \hlstr{"value"}\hlstd{))}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=0.5\linewidth]{figure/mantel} 
