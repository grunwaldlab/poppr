...
\title{Poppr 1.0.0: An R package for genetic analysis of populations with mixed (clonal/sexual) reproduction}
...
The calculation for the distance between two individuals at a single locus with $a$ allelic states is as follows \footnote{Haploid individuals under this construct will have $d == (0 | 2)$, but the ratio of the variance is the same as if $d == (0 | 1)$}:
\beq
\label{eq:ia_d}
...
\item bruvo.boot(pop, replen = 1, add = TRUE, loss = TRUE, B = 100, tree = "upgma", showtree = TRUE, cutoff = NULL, quiet = FALSE) (Section \ref{index:trees:bruvo.boot})
