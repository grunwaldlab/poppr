...
print_args <- function(arg_name, arg_list){
  arg_value <- arg_list[[arg_name]]  
  if (missing(arg_value)){
    return(arg_name)
    #cat(paste0(arg_name, ", "))
  } else {
    if (arg_name == "glim"){
      arg_value <- 'c(0, 0.8)'
    } else if (arg_name == "cutoff" & length(arg_value) > 1){
      arg_value <- '2/nInd(pop)'
    } else if (is.character(arg_value)){
      arg_value <- paste0('"', arg_value, '"')
    } else if (is.null(arg_value)) {
      arg_value <- "NULL"
    ...
    #cat(paste0(arg_name, " = ", arg_value, comma))
  }
}
print_command <- function(funk){
  forms <- formals(funk)
  arg_length <- length(forms)
  all_args <- vapply(names(forms), print_args, character(1), forms)
  if (arg_length > 1){
    all_args <- c(paste0(all_args[-arg_length], ","), all_args[arg_length])
    cat(paste0(funk,"(",all_args[1]), 
      all_args[c(-1,-arg_length)], 
      paste0(all_args[arg_length],")"), fill = 80)
  } else {
    cat(paste0(funk,"(",all_args,")"), fill = 80)
  }
}
@
