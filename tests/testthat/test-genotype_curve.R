...
test_that("genotype_curve's drop functionality works by default", {
  skip_on_cran()
  expect_message(
    x <- genotype_curve(dat[-nInd(dat)], plot = FALSE, quiet = TRUE),
    "Dropping monomorphic loci: w, y"
  )
  expect_message(
    y <- genotype_curve(dat[-nInd(dat2)], plot = FALSE, quiet = TRUE),
    "Dropping monomorphic loci: w, y"
  )
...
test_that("genotype_curve's drop can be turned off", {
  ...
  expect_output(
    x <- genotype_curve(dat[-nInd(dat)], drop = FALSE, plot = FALSE),
    "/4 loci"
  )
  expect_output(
    y <- genotype_curve(dat[-nInd(dat2)], drop = FALSE, plot = FALSE),
    "/4 loci"
  )
