...
% \begin{figure}[h!]
%   \centering
%   \caption{\footnotesize Output for population 1 of the data set ``Aeut"}
%   \label{ia_demo_fig2}
...
\tab\tab Use this function to display a gradient of grey values based on user-defined parameters. This function does not use your data to draw the gradient, it is simply a tool for you to decide what grey scale adjustment you might want to use to represent your data on a minimum spanning network. 
\begin{quote}
Default Command:\\
...
\begin{itemize}
  ...
  \item \texttt{gadj -} This is an integer greater than zero used to adjust the scaling factor for the grey curve. Since very small changes in the grey scale are not easily precieved, it's useful to be able to adjust the grey scale to be able to show you the weights of each edge. For example, a population with most weights less than 0.2, you might want to set \texttt{gadj = 10} to exaggerate the grey scale. 
